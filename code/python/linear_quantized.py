...
class apxLinear(Function): # Linear layer
    @staticmethod
    def forward(ctx, input, weight, bias, input_scale, i_zero_point, w_scale, w_zero_point, apx):
        ...
        output = apxop.mm(input, weight.t().contiguous(), apx, 8).float().mul_(input_scale * w_scale)
...
class apxLinearF(Function): # Linear function
    @staticmethod
    def forward(ctx, input, weight, input_scale, i_zero_point, w_scale, w_zero_point, apx):
        ...
        output = apxop.mm(input, weight, apx, 8).float().mul_(input_scale * w_scale)
